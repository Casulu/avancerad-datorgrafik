...
void parseNodes(aiNode *root_node, MaterialVector &materials, std::stack<glm::mat4> &transformStack, std::shared_ptr<RenderItem> &node, const aiScene *aiScene)
{
...
std::shared_ptr<RenderItem> vr::load3DModelFile(const std::string &filename)
{
    ...
    std::shared_ptr<RenderItem> node = std::shared_ptr<RenderItem>(new RenderItem);
    ...
    try
    {
        ...
        for (rapidxml::xml_node<> *node_node = root_node->first_node("node"); node_node; node_node = node_node->next_sibling())
        {
            ...
            if (transform)
            {
                ...
                // Now create the node
                std::shared_ptr<RenderItem> loadedNode = vr::load3DModelFile(path);
                if (!loadedNode)
